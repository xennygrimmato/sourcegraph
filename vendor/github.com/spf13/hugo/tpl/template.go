...
func (t *GoHTMLTemplate) AddTemplateFile(name, baseTemplatePath, path string) error {
	...
	case ".ace":
		...
		innerContent, err := readFile(path)
...
func (t *GoHTMLTemplate) loadTemplates(absPath string, prefix string) {
	vfs := source.WalkableAferoFS{hugofs.SourceFs}
	w := fs.WalkFS(absPath, vfs)

	for w.Step() {
		if w.Err() != nil {
			log.Printf("loadTemplates(%s, %s): %s", absPath, prefix, w.Err())
			continue
		}
		...
		if !fi.IsDir() {
			...
			if filepath.Ext(path) == ".ace" && !strings.HasSuffix(filepath.Dir(path), "partials") {
				...
				needsBase, err := helpers.FileContains(path, aceTemplateInnerMarker, vfs)
				if err != nil {
					log.Println(err)
					continue
				}
				if needsBase {
					...
					for _, pathToCheck := range pathsToCheck {
						if ok, err := helpers.Exists(pathToCheck, vfs); err == nil && ok {
