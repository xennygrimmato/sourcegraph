...
function initTernServer(ctx) {
    localFiles = Object.keys(ctx.sources);
    var ternOptions = {
        ...
        getFile: function(file) {
			const f = ctx.sources[file];
			if (f) return f.toString();
			logger.warn("getFile failed to load file %s (not in sources list)", file);
			return null;
        },
        plugins: {
            node: true,
    ...
    ctx.origins.forEach(function(file) {
    ...
    return {tern: ternServer, out: out};
}
module.exports.initTernServer = initTernServer;
...
function emitModuleRef(moduleData) {
    ...
    if (externalRepo) {
        ...
        if (data.origin && !isStandardOrigin(data.origin)) {
            var ref = {
        ...
        ExportNamedDeclaration: function(node, nodeInfo, c) {
            for (var i = 0; i < node.specifiers.length; i++) {
                if (node.specifiers[i].local) {
                    c(node.specifiers[i].local, initNodeInfo("var"));
                }
            }
        ...
        ImportDeclaration: function(node, nodeInfo, c) {
            for (var i = 0; i < node.specifiers.length; ++i) {
                ...
                if (node.specifiers[i].local) {
                    ...
                    importVars[pathForId] = {pathForDef: pathForDef, externalRepo: externalRepo, data : data};
...
function processIdent(node, nodeInfo, name, path) {
    ...
    if (path) {
        ...
        pathForDef = scope.formPathForScope(node);
    } else {
	...
	out.Refs.add(Object.assign({}, data, {typeInfo: typeInfo}));
