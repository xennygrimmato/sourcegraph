...
func serveRepoRefresh(w http.ResponseWriter, r *http.Request) error {
	...
	repoupdater.Enqueue(repo.ID, authInfo.UserSpec())
...
func serveRepoBranches(w http.ResponseWriter, r *http.Request) error {
	...
	branches, err := cl.Repos.ListBranches(ctx, &sourcegraph.ReposListBranchesOp{Repo: repo.ID, Opt: &opt})
	if err != nil {
		return err
	}
...
func serveRepoTags(w http.ResponseWriter, r *http.Request) error {
	...
	tags, err := cl.Repos.ListTags(ctx, &sourcegraph.ReposListTagsOp{Repo: repo.ID, Opt: &opt})
	if err != nil {
		return err
	}
